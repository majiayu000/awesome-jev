...
网页目录由 `catalog/FULL.md` 生成，中文用途说明复用 `README_zh.md`。修改这两份来源后运行 `python3 scripts/build_site.py`，同时提交首页、`docs/categories/` 分类页与 `docs/sitemap.xml`；用 `python3 scripts/build_site.py --check` 检查是否同步。`docs/data/entries.json` 保留历史收集记录，不再作为网页展示数据。

修改示例后运行 `python3 -m unittest discover -s examples -v`，默认测试不调用真实 API。

## English summary

Provide a public link and a plain-language description of the use case. Attribute claims to their sources, distinguish commentary from experiments, and state when results have not been reproduced here. Keep both READMEs aligned. Use the category pages for additional resources and the dated updates for news. After editing `catalog/FULL.md` or `README_zh.md`, run `python3 scripts/build_site.py` and commit the generated homepage, category pages and sitemap too.
