...
def build():
    ...
    for line in (ROOT / 'catalog/FULL.md').read_text().splitlines():
        if match := re.fullmatch(r'<a id="([^"]+)"></a>', line):
            ...
            cards.append(f'<article class="card" data-category="{html.escape(category_id, quote=True)}"><div class="resource-number">{len(cards)+1:03d}</div><div class="resource-body"><div class="category">{html.escape(category)}</div><h3><a href="{html.escape(links[0][1], quote=True)}">{html.escape(name)}</a></h3><p>{html.escape(description)}</p></div><div class="resource-links">{link_html}</div></article>')
    if not cards:
    ...
    buttons = ''.join(f'<button type="button" data-category="{key}" aria-pressed="false">{html.escape(value)} <span>↗</span></button>' for key, value in categories.items())
